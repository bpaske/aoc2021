...
@cache
def calculate_wins(game_state, player1_turn):
    ...
    for roll, roll_count in roll_frequencies.items():
        if(player1_turn):
            ...
            next_state = (new_position, new_score, player2_position, player2_score)
        else:
            new_position = (player2_position + roll) % 10
            new_score = ( player2_score + new_position ) if new_position !=0 else (player2_score + 10)
            next_state = (player1_position, player1_score, new_position, new_score)

        additional_player1_wins, additional_player2_wins= calculate_wins(next_state, not player1_turn)
        player1_wins += roll_count*additional_player1_wins
        player2_wins += roll_count*additional_player2_wins

    return player1_wins, player2_wins

print(calculate_wins((4,0,8,0), True))
